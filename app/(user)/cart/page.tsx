...
export default function CartPage() {
      ...
      Object.keys(stockLevels).forEach((itemId) => {
        ...
        if (previousStock !== undefined && currentStock !== previousStock) {
          ...
          if (cartItem) {
            ...
            if (stockChange < 0) {
              toast.error(
                `Stock Alert: ${itemName} - ${Math.abs(
                  stockChange
                )} units removed from inventory (Now: ${currentStock})`,
                { duration: 5000 }
              );
  ...
  useEffect(() => {
    ...
    if (
      userRole === "buyer" &&
      (isLoadingItems || Object.keys(stockLevels).length === 0)
    ) {
      return;
    }
  ...
  const handleInputBlur = (itemId: string, item: CartItem) => {
    ...
    if (!validation.isValid) {
      ...
      if (
        ...
        `Requested quantity exceeds available stock (${maxQuantity} available)`
      ) {
    ...
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          <Recycle className="w-8 h-8 text-green-600 flex-shrink-0" />
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800 tracking-tight">
            {t("cart.confirmItems")}
          </h1>
        </div>

        {/* Right: Real-time section */}
        {userRole === "buyer" && (
          <div className="flex items-center gap-2 text-sm text-green-600 sm:ml-auto">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
            <span className="whitespace-nowrap">Real-time inventory</span>
            <button
      ...
      {cart.length === 0 ? (
        ...
        <>
          <div
            className={`rounded-xl p-4 mb-6 grid grid-cols-1 ${
          ...
          >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-orange-700 mt-1">
                      You can continue shopping, but please adjust quantities to
                      available stock levels before checkout.
                    </p>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
              ...
              <Button
                ...
                className="border border-gray-300 text-gray-700 hover:bg-gray-50 w-full sm:w-auto"
              >
              ...
              <div className="flex flex-col items-stretch sm:items-end w-full sm:w-auto">
                ...
                >
                  <Button
                    ...
                    className={`flex items-center justify-center gap-2 px-6 py-2 rounded-lg transition-colors shadow-md hover:shadow-lg w-full sm:w-auto ${
                      totalPrice < 100 || hasExceedsStockItems
                ...
                {hasExceedsStockItems && (
                  <p className="text-xs text-orange-600 mt-1 text-center sm:text-right">
                    Please adjust quantities to available stock levels to
                    continue
                  </p>
                )}
              </div>
