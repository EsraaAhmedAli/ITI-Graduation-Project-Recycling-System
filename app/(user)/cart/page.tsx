...
export default function CartPage() {
  ...
  const queryClient = useQueryClient(); // Add query client for manual invalidation
  ...
  // Use React Query with aggressive real-time polling for inventory data
  const {
    data: itemsData,
    ...
    refetchInterval: 2000, // More frequent polling - every 2 seconds
  ...
  // Real-time stock change detection and notification with better synchronization
  ...
  // Force re-evaluation when data updates
  ...
  // Additional mechanisms to ensure fresh data
  useEffect(() => {
    // Invalidate and refetch when component mounts or cart changes
    if (userRole === "buyer") {
      queryClient.invalidateQueries(['items']); // Invalidate items cache
    }
  }, [cart.length, userRole, queryClient]);

  // Window focus handler for immediate refresh
  useEffect(() => {
    const handleFocus = () => {
      if (userRole === "buyer") {
        refetchItems();
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && userRole === "buyer") {
  ...
  }, [userRole, refetchItems, queryClient]);
  ...
  const coin = locale === "en" ? "EGP" : "ج.م";
    ...
    useEffect(() => {
    if (dataUpdatedAt && dataUpdatedAt !== lastDataUpdate) {
      setLastDataUpdate(dataUpdatedAt);
  ...
  }, [stockLevels, cart, userRole, dataUpdatedAt, lastDataUpdate, previousStockLevels, locale]);
                      ...
                      {userRole === "buyer" && (
                        <div className="text-xs mt-1 flex items-center gap-2">
                          ...
                          <button
                            onClick={() => {
                              refetchItems();
                              toast.info('Stock refreshed', { duration: 1500 });
                            }}
