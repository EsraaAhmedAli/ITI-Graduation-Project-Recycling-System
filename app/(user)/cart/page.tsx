...
export default function CartPage() {
  ...
  // Only call useGetItems when userRole is "buyer"
  const {
    data: itemsData,
    ...
    enabled: userRole === "buyer", // This is the key change - only enable the query when userRole is "buyer"
  });

  // Alternative approach if your useGetItems doesn't support 'enabled' option:
  // You can wrap it in a conditional hook pattern
  /*
  const shouldFetchItems = userRole === "buyer";
  const {
    data: itemsData,
    isLoading: isLoadingItems,
    error: itemsError,
  } = shouldFetchItems ? useGetItems({
    currentPage: 1,
    itemsPerPage: 10000,
    userRole: userRole || "buyer",
  ...
  */
  ...
  // Show loading state while fetching inventory data - only for buyers
