...
import { Order, OrdersResponse, SafetyReport } from "@/components/Types/orders.type";
...
function ProfileContent() {
  ...
  // Get all safety reports from orders
  const getAllSafetyReports = (): SafetyReport[] => {
    return allOrders
      .filter(order => order.safetyReports && order.safetyReports.length > 0)
      .flatMap(order => order.safetyReports || []);
  };

  const getAllOrders = async (): Promise<void> => {
    try {
      setLoading(true);
      // Fetch orders with cancellation reasons and safety reports
      const res = await api.get<OrdersResponse>("/orders?includeCancellation=true&includeSafetyReports=true");
      setAllOrders(res.data.data);
      console.log("Orders with cancellation and safety data:", res.data.data);
      
      // Debug: Check if orders have safety reports
      const ordersWithSafetyReports = res.data.data.filter(order => order.safetyReports && order.safetyReports.length > 0);
      console.log("Orders with safety reports:", ordersWithSafetyReports);
      console.log("Total safety reports:", getAllSafetyReports().length);
...
const closeModal=()=>{
  ...
  const tabs = ["incoming", "completed", "cancelled", "safety"];
 ...
 modalOpen={modalOpen} closeModal={closeModal} totalPoints={userPoints?.totalPoints}/>
        ...
        {activeTab === "safety" ? (
          <>
            {loading ? (
              <Loader title=" safety reports..." />
            ) : getAllSafetyReports().length === 0 ? (
              <p className="text-center text-gray-500">No safety reports yet.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {getAllSafetyReports().map((report) => (
                  <div
                    key={report._id}
                    className="border rounded-xl p-4 bg-orange-50 shadow-sm space-y-2"
                  >
                    ...
                    <div className="bg-white p-3 rounded-lg">
                      <p className="text-sm font-semibold text-orange-800 mb-2">
                        Issue Type: {report.type.replace('_', ' ').toUpperCase()}
                      </p>
              ...
              >
                ...
                {activeTab === "incoming" && order.status !=='assigntocourier' && (
                  <button
                    ...
                    className="mt-2 px-3 py-2 text-sm text-white bg-rose-500 hover:bg-rose-600 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105"
                  >
