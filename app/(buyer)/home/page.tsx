...
const BLUR_DATA =
  "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAwIiBoZWlnaHQ9IjIwMCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwJSIgaGVpZ2h0PSIxMDAlIiBmaWxsPSIjZjBmMGYwIi8+PC9zdmc+";
  ...
  ({
    ...
    return (
      <div className="relative">
        <Link
          onClick={() => markThatFromMarketPlace(user?.role || "")}
          href={`/marketplace/${encodeURIComponent(item.name.en)}`}
          passHref
        >
          <div
            style={{ background: "var(--color-card-home)" }}
            className={`bg-gray-50 rounded-xl p-4 border border-gray-100 hover:border-emerald-200 transition-colors duration-200 h-full relative ${
          ...
          >
            ...
            <div
              ...
              className="relative aspect-square w-full mb-3 bg-white rounded-lg overflow-hidden flex items-center justify-center"
            >
  ...
  ({
    ...
    return (
      <Link
        ...
        style={{ background: "var(--color-card-home)" }}
        key={index}
        href={`/marketplace/${encodeURIComponent(material.name.en)}`}
        passHref
      >
        <div
          style={{ backgroundColor: "var(--color-card)" }}
          className="rounded-xl p-4 border border-gray-100 hover:border-emerald-200 transition-colors duration-200"
        >
          <div
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm" style={{ color: "var(--text-gray-700)" }}>
              ...
              >
                {convertNumber(material.totalRecycled)}
              </span>
              {material.unit === "pieces" ? t("common.piece") : t("common.kg")}{" "}
              {t("common.sold")}
            </span>
          </div>
        </div>
...
const RecyclingBanner = memo(
  ({ statsData, t }: { statsData: Array<any>; t: (key: string) => string }) => (
          ...
          <div className="text-center lg:text-start lg:w-1/2">
            <h2 className="text-xl md:text-2xl font-bold text-white mb-3">
              <span className="inline-block bg-white/20 px-2 py-1 rounded-full text-emerald-100 text-xs mb-2">
                {t("navbar.title").split(" ")}
              </span>
              <br />
              {t("recyclingBanner.title")
                .split(t("recyclingBanner.titleHighlight"))
                  ...
                  return (
                    <span key={index}>
                      ...
                      <span className="text-yellow-300">
                        {t("recyclingBanner.titleHighlight")}
                      </span>
                    </span>
                  );
                })}
...
export default function BuyerHomePage() {
  ...
  const formatQuantity = useCallback(
    (quantity: number, unit: number) => {
      ...
      if (quantity < 10) {
        return `${t("common.only")} ${convertNumber(quantity)} ${unitText} ${t(
          "common.left"
        )}`;
      }
  ...
  const fetchItems = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const response = await api.get(
        `/categories/get-items?page=1&limit=8&role=buyer`,
        { signal }
      );
  ...
  const filteredItems = useMemo(() => {
    ...
    return items
      .filter((item) => {
        const itemName =
          item.name[locale]?.toLowerCase() || item.name.en?.toLowerCase() || "";
        const categoryName =
          item.categoryName[locale]?.toLowerCase() ||
          item.categoryName.en?.toLowerCase() ||
          "";

        const matchesSearch =
          !term || itemName.includes(term) || categoryName.includes(term);
