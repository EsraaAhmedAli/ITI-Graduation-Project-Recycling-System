...
//   useEffect(() => {
//     if (item) {
//       setSelectedQuantity(item.measurement_unit === 1 ? 0.25 : 1);
//     }
//   }, [item]);
...
//                       Math.max(
//                         1,
//                         prev - (item.measurement_unit === 1 ? 0.25 : 1)
//                       )
//                     )
//                   }
...
export default function ItemDetailsPage() {
  ...
  // Initialize selectedQuantity when item is loaded or changes
  useEffect(() => {
    if (item) {
      const initialQuantity = item.measurement_unit === 1 ? 0.25 : 1;
      // Only update if the value would change
      if (selectedQuantity !== initialQuantity) {
        setSelectedQuantity(initialQuantity);
      }
    }
  }, [item?.measurement_unit]); // Only run when measurement_unit changes
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              className={`w-full py-3 px-6 rounded-lg font-medium flex items-center justify-center space-x-2 transition-colors ${
                isOutOfStock || loadingItemId === item._id
                  ? "bg-green-600 text-green-500 cursor-not-allowed"
                  : "bg-green-600 hover:bg-green-700 text-white shadow-md hover:shadow-lg"
              }`}
            >
              {loadingItemId === item._id ? (
