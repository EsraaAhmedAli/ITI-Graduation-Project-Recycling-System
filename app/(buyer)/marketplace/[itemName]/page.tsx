...
const OptimizedImage = memo(({ 
  ...
  const optimizedSrc = useMemo(() => {
    if (src.includes("cloudinary.com")) {
      return src.replace("/upload/", "/upload/c_fit,w_600,q_auto,f_auto,dpr_auto/");
    }
    return src;
  }, [src]);
    ...
    <div className="pt-4 border-t border-gray-200">
      <div className="flex items-center justify-between mb-2">
        ...
        >
          {isOutOfStock
            ? t("common.outOfStock")
            : `${item?.quantity} ${getMeasurementText(item.measurement_unit)}`}
        </span>
      </div>
      ...
      {isLowStock && !isOutOfStock && (
        <p className="text-xs text-amber-600 flex items-center">
          ...
          {t("common.lowStockWarning", {
            ...
            unit: getMeasurementText(item.measurement_unit),
            defaultValue: `Low stock - only ${item.quantity} ${getMeasurementText(
...
const EnvironmentalBenefits = memo(({ selectedQuantity, t }: { 
  selectedQuantity: number; 
  t: (key: string, params?: any) => string;
}) => {
...
export default function ItemDetailsPage() {
  ...
  // Optimized cart sync
  const syncCartWithChanges = useCallback((quantity: number) => {
    if (!item) return;
    ...
    if (cartItem) {
      startTransition(() => {
        updateCartState(
          cart.map((ci) => (ci._id === item._id ? { ...cartItem, quantity } : ci))
        );
      });
    }
  ...
  }, [item, computedValues, cart, convertToCartItem, updateCartState, addToCart, selectedQuantity]);
  ...
  const itemName_display = typeof item.name === 'string' ? item.name : item.name[locale];
  const categoryName_display = typeof item.categoryName === 'string' 
    ? item.categoryName 
              ...
              <>
                ...
                aria-label="add or delete"
                  ...
                  className={`w-full py-3 px-6 rounded-lg font-medium flex items-center justify-center space-x-2 transition-colors duration-150 ${
                    computedValues.isOutOfStock || loadingItemId === item._id || !!inputError || isPending
