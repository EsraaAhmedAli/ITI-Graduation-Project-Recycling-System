...
const StatusLegendItem = memo<{ status: string; count: number; color: string }>(
  ({ status, count, color }) => {
    ...
    const translateStatus = (status: string): string => {
      // Try to get translation from profile.orders.status first
      const statusTranslation = t(`profile.orders.status.${status}`);
      if (
        statusTranslation &&
        statusTranslation !== `profile.orders.status.${status}`
      ) {
        return statusTranslation;
      }

      // Try to get translation from root-level orderStatuses
      const rootStatusTranslation = t(`orderStatuses.${status}`);
      if (
        rootStatusTranslation &&
        rootStatusTranslation !== `orderStatuses.${status}`
      ) {
        return rootStatusTranslation;
      }

      // Fallback to direct status translation
      const directTranslation = t(status);
      if (directTranslation && directTranslation !== status) {
...
const OrderStatusChart = memo<OrderStatusChartProps>(
  ({ orderStatus, loading }) => {
    const { t } = useLanguage(); // Get translation function
    ...
    const chartData = useMemo(() => {
      ...
      return {
        labels: statuses.map((status) => translateStatus(status)), // Translate status labels
        datasets: [
          {
            data: counts,
            backgroundColor: statuses.map(
              (status) => STATUS_COLOR_MAP[status] || "#d1d5db"
            ),
            borderWidth: 0,
            hoverBackgroundColor: statuses.map(
              (status) => (STATUS_COLOR_MAP[status] || "#09c") + "CC"
            ),
            hoverBorderWidth: 2,
    ...
    }, [normalizedStatus, translateStatus]);
    ...
    return (
      <div className="bg-white rounded-xl p-4 md:p-6 shadow border border-green-100 flex-1 flex flex-col">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs text-green-400">{t("overview")}</span>
        </div>
        ...
        {!loading && totalOrders > 0 && (
          <div className="mt-4 pt-3 border-t border-green-100 text-xs text-gray-600">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <span className="font-medium">{t("mostCommon")}: </span>
                <span className="text-green-700">
                  {Object.entries(orderStatus).sort(
                    (a, b) => b[1] - a[1]
                  )[0]?.[0]
                    ? translateStatus(
                        Object.entries(orderStatus).sort(
