...
export default function ItemsModal({ show, onclose, selectedOrderItems, userRole }) {
        ...
        {selectedOrderItems && selectedOrderItems.length > 0 ? (
          <div className="space-y-4">
            {selectedOrderItems.map((item: any, index: number) => {
                console.log(item.itemName, item.price, priceWithMarkup(item.price || 0, userRole));
              const price = priceWithMarkup(item.price || 0, userRole);

              return (
                <div key={index} className="bg-yellow-50 rounded-xl p-5 border border-blue-100">
