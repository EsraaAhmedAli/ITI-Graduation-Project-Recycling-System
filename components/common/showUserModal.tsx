...
export default function CourierOrderDetailsModal({ show, onClose, order }: Props) {
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Complete order information</p>
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      <span className="text-sm font-medium text-gray-600 min-w-[60px]">City:</span>
                      <span className="text-gray-900">{address?.city}</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="text-sm font-medium text-gray-600 min-w-[60px]">Area:</span>
                      <span className="text-gray-900">{address?.area}</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="text-sm font-medium text-gray-600 min-w-[60px]">Street:</span>
                      <span className="text-gray-900">{address?.street}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      <span className="text-sm font-medium text-gray-600 min-w-[70px]">Building:</span>
                      <span className="text-gray-900">{address?.building}</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="text-sm font-medium text-gray-600 min-w-[70px]">Floor:</span>
                      <span className="text-gray-900">{address?.floor}</span>
                    </div>
                    <div className="flex items-start gap-2">
                      <span className="text-sm font-medium text-gray-600 min-w-[70px]">Apartment:</span>
                      <span className="text-gray-900">{address?.apartment}</span>
                    </div>
                  </div>
                </div>
                {address?.landmark && (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-gray-800">Items to Collect ({items?.length || 0})</h3>
              </div>
              
              <div className="space-y-3">
                {items?.map((item: any, idx: number) => (
                  <div key={idx} className="bg-gray-50 rounded-lg border border-gray-200 p-4 hover:shadow-md transition-all duration-200">
                        ...
                        <div className="flex flex-wrap gap-2 text-sm">
                          ...
                          <div className="flex items-center bg-green-100 px-3 py-1.5 rounded-full border border-green-200">
                            <svg className="w-4 h-4 mr-1.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16l3-1m-3 1-3-1" />
                            </svg>
